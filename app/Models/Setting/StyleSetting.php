...
class StyleSetting
{
	...
	public static function getValues($value, $disk)
	{
		if (empty($value)) {
			...
			$value['login_bg_image_path'] = config('larapen.admin.login_bg_image');
			...
			foreach ($value as $key => $item) {
				...
				if ($key == 'login_bg_image_path') {
					if (empty($value['login_bg_image_path']) || !$disk->exists($value['login_bg_image_path'])) {
			...
			if (!array_key_exists('login_bg_image_path', $value)) {
				$value['login_bg_image_path'] = config('larapen.admin.login_bg_image');
			}
		...
		// login_bg_image_url
		$loginBgImage = $value['login_bg_image_path'] ?? $value['login_bg_image'] ?? config('larapen.admin.login_bg_image', '');
		$value['login_bg_image_url'] = thumbService($loginBgImage, false)->resize('bg-body')->url();
