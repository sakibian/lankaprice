...
{
	if (empty($pluginSetting['key']) || empty($pluginSetting['name'])) {
		$message = 'The columns "key" and "name" are required';
		throw new CustomException($message);
	}
	
	// Remove the plugin setting (for security)
	dropPluginSetting($pluginSetting['key']);
	
	// Get the setting's position
	$lft = getNextSettingPosition();
	$rgt = $lft + 1;
	
	$pluginSetting['description'] = $pluginSetting['description'] ?? $pluginSetting['name'];
	$pluginSetting['field'] = null;
	$pluginSetting['value'] = null;
	$pluginSetting['parent_id'] = 0;
	$pluginSetting['lft'] = $lft;
	$pluginSetting['rgt'] = $rgt;
	$pluginSetting['depth'] = 1;
 ...
 * @return void
 */
function dropPluginSetting(string $key): void
{
	// Remove the plugin setting
