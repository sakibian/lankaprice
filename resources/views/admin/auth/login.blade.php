            ...
            <div class="col-12">
                ...
                <form class="form-horizontal mt-3" id="loginform" action="{{ admin_url('login') }}" method="post">
                    ...
                    @if (config('settings.sms.enable_phone_as_auth_field') == '1')
                        ...
                        @endphp
                        <div class="row mb-3 auth-field-item{{ $phoneRowError ?? '' }}">
                            <div class="col-12 pb-1">
                            ...
                            <div class="">
                                <input id="phone" name="phone"
                                       type="tel"
                                       class="form-control{{ $phoneFieldError ?? '' }}"
                    ...
                    @php
                        if (isset($errors)) {
							...
							$passwordRowError = $passwordHasError ? ' has-danger' : '';
							$passwordFieldError = $passwordHasError ? ' form-control-danger' : '';
							$passwordError = $errors->first('password');
						}
                    @endphp
                    <div class="row mb-3{{ $passwordRowError ?? '' }}">
                        <div class="input-group">
                            <span class="input-group-text"><i class="fa-solid fa-lock"></i></span>
                            <input id="password" name="password"
                                   type="password"
                                   class="form-control{{ $passwordFieldError ?? '' }}"
                                   placeholder="{{ trans('admin.password') }}"
                                   autocomplete="new-password"
                            >
                        </div>
                        @if (isset($passwordHasError) && $passwordHasError)
                        ...
                        <div class="d-flex">
                            ...
                            <div class="ms-auto">
                                <a href="javascript:void(0)" id="to-recover" class="text-muted float-end">
                                    <i class="fa-solid fa-lock me-1"></i> {{ trans('admin.forgot_your_password') }}
                                </a>
                            </div>
                        </div>
                    </div>
