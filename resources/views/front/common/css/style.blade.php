...
@if (
	...
	|| !empty(config('settings.style.body_text_color'))
...
@if (!empty(config('settings.listings_list.grid_view_cols')))
	@php
		$gridViewCols = (int)config('settings.listings_list.grid_view_cols');
	...
	@if (config('lang.direction') == 'rtl')
		...
		/* Items' Border */
		html:not([theme="dark"]) .posts-wrapper.make-grid .item-list:nth-child(4n+4),
		html:not([theme="dark"]) .category-list.make-grid .item-list:nth-child(4n+4) {
			border-right: 1px solid var(--border-color);
		}
		
		html:not([theme="dark"]) .category-list.make-grid .item-list:nth-child({{ $gridViewCols }}n+{{ $gridViewCols }}) {
			border-left: none;
		}
		html:not([theme="dark"]) .category-list.make-grid .item-list:nth-last-child(-n+{{ $gridViewCols }}),
		html:not([theme="dark"]) .category-list:not(.make-grid) .item-list:last-child {
			border-bottom: none;
		}
		html:not([theme="dark"]) .row-featured-category .category-list.make-grid .item-list:nth-last-child(-n+{{ $gridViewCols }}),
		html:not([theme="dark"]) .row-featured-category .category-list:not(.make-grid) .item-list:last-child {
			border-bottom: 1px solid var(--border-color);
		}
		html:not([theme="dark"]) .category-list.make-grid .item-list:nth-last-child(-n+{{ $gridViewCols }}):nth-child({{ $gridViewCols }}n+1) {
			border-bottom-right-radius: 7px;
		}
		html:not([theme="dark"]) .category-list.make-grid .item-list:nth-last-child(-n+{{ $gridViewCols }}):nth-child({{ $gridViewCols }}n) {
		...
		@media (max-width: 767px) {
			html:not([theme="dark"]) .category-list.make-grid .item-list:nth-child(2n+2) {
				border-left: none;
			}
		}
	@else
