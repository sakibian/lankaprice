		...
		<div class="modal-content">
			...
			<div class="modal-header px-3">
				<h4 class="modal-title">
					<i class="fa-solid fa-envelope"></i> {{ t('contact_advertiser') }}
				</h4>
			...
			<form role="form" method="POST" action="{{ $actionUrl }}" enctype="multipart/form-data">
				...
				<div class="modal-body">
					...
					@if ($catType == 'job-offer')
						@php
							$filePathError = (isset($errors) && $errors->has('file_path')) ? ' is-invalid' : '';
						@endphp
						<div class="mb-3 required" {!! (config('lang.direction')=='rtl') ? 'dir="rtl"' : '' !!}>
							<label class="control-label{{ $filePathError }}" for="file_path">{{ t('Resume') }} </label>
							<input id="filePath" name="file_path" type="file" class="file{{ $filePathError }}">
