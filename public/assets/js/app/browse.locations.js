...
function browseLocations(params) {
	...
	/* Reorder the admin. division & the city list */
	const adminListReorder = new BsRowColumnsReorder('#modalAdminList', {defaultColumns: 4});
	const cityListReorder = new BsRowColumnsReorder('#modalCityList', {defaultColumns: 4});
	...
	ajax.done(function (xhr) {
		...
		try {
			...
			if (xhr.isCity) {
				cityListReorder.addNewContainers(locationsListEl[0]);
			} else {
				adminListReorder.addNewContainers(locationsListEl[0]);
			}
		} catch (e) {
